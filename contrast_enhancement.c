...
float*** enhance_contrast (float*** input, int M_in, int N_in)
{
    ...
    // Prompt the user.
    printf("Quel étirement d'histogramme souhaitez-vous effectuer ? (1) pour");
    printf(" un etirement linéaire, (2) pour le 3%% min-max etirement linéaire.\n");
    scanf("%d", &option);

    switch (option) {
        case 2:
            printf("Exécution de l'étirement linéaire 3%% min-max...");
            return min_max_stretch(input, M_in, N_in, 0.03);
        default:
            printf("Exécution de l'étirement d'histogramme linéaire...");
